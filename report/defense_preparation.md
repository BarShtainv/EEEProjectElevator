...
5. Claims are disciplined: representative literature, host timing, model boundaries, and software-only evidence are not overstated.

## Five limitations to acknowledge confidently

1. The implementation is host software; no physical RFID reader, RFID electronics, embedded controller, output circuit, or elevator interface was tested.
2. The `LF` and `HF` labels, 26-bit frame, floor mask, watchdog, and logical outputs are project-defined abstractions.
...
5. No real-time, electrical, security-hardening, safety, certification, or deployment-readiness conclusion is supported.
...
The result is a reproducible and testable authorization reference design. Its boundary matters: it is not a physical RFID system, does not operate an elevator, and does not prove real-time or safety behavior.
