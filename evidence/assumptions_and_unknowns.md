...
| PRD-002 | possible proposed-design element | A later simulator may use Wiegand-26 as an initial reference input format if approved and supported by authoritative protocol literature. | Project plan names Wiegand-26 for a future software package; no product evidence establishes support. | Separates future simulation scope from product claims. | Approved requirements and authoritative Wiegand source. | deferred; source gap open | Blocks protocol implementation until supported; non-blocking for evidence architecture. |
